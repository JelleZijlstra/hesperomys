...
import { ArticleSpellingSelections_article } from "./__generated__/ArticleSpellingSelections_article.graphql";
import { ArticleSpellingSelections_articleInner } from "./__generated__/ArticleSpellingSelections_articleInner.graphql";
import { ArticleSpellingSelectionsDetailQuery } from "./__generated__/ArticleSpellingSelectionsDetailQuery.graphql";
...
interface ArticleSpellingSelectionsInnerProps {
  articleInner: ArticleSpellingSelections_articleInner;
  title?: string;
...
> {
  render() {
    ...
    if (
      ...
      articleInner.spellingSelections.edges.length === 0
    ) {
      return null;
    }
    return (
      <>
        {!hideTitle && <h3>{title || "SpellingSelections"}</h3>}
        {subtitle}
        <NameList
...
const ArticleSpellingSelectionsContainer = createPaginationContainer(
  ArticleSpellingSelectionsInner,
  {
  ...
  {
    getConnectionFromProps: (props) => props.articleInner.spellingSelections,
    ...
    query: graphql`
      query ArticleSpellingSelectionsPaginationQuery(
        $count: Int!
        $cursor: String
        $oid: Int!
        $showLocationDetail: Boolean!
...
interface ArticleSpellingSelectionsProps {
  article: ArticleSpellingSelections_article;
  title?: string;
  subtitle?: JSX.Element;
  hideTitle?: boolean;
  numToLoad?: number;
...
class ArticleSpellingSelections extends React.Component<
  ArticleSpellingSelectionsProps,
  {
    showLocationDetail: boolean;
    showCitationDetail: boolean;
    showCollectionDetail: boolean;
...
> {
  constructor(props: ArticleSpellingSelectionsProps) {
    super(props);
    const {
      showLocationDetail,
      showCitationDetail,
  ...
  renderInner(
    ...
    return (
      <ArticleSpellingSelectionsContainer
  ...
  render() {
    ...
    if (
      ...
      return (
        <QueryRenderer<ArticleSpellingSelectionsDetailQuery>
          environment={environment}
          query={graphql`
            query ArticleSpellingSelectionsDetailQuery(
              $oid: Int!
              $showLocationDetail: Boolean!
              $showCitationDetail: Boolean!
              $showCollectionDetail: Boolean!
