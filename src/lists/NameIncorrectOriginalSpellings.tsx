...
import { NameIncorrectOriginalSpellings_name } from "./__generated__/NameIncorrectOriginalSpellings_name.graphql";
import { NameIncorrectOriginalSpellings_nameInner } from "./__generated__/NameIncorrectOriginalSpellings_nameInner.graphql";
import { NameIncorrectOriginalSpellingsDetailQuery } from "./__generated__/NameIncorrectOriginalSpellingsDetailQuery.graphql";
...
interface NameIncorrectOriginalSpellingsInnerProps {
  nameInner: NameIncorrectOriginalSpellings_nameInner;
  title?: string;
...
class NameIncorrectOriginalSpellingsInner extends React.Component<NameIncorrectOriginalSpellingsInnerProps> {
  render() {
    ...
    if (
      ...
      nameInner.incorrectOriginalSpellings.edges.length === 0
    ) {
      return null;
    }
    const inner = (
...
const NameIncorrectOriginalSpellingsContainer = createPaginationContainer(
  NameIncorrectOriginalSpellingsInner,
  {
    nameInner: graphql`
  ...
  },
  {
    ...
    query: graphql`
      query NameIncorrectOriginalSpellingsPaginationQuery(
        $count: Int!
        $cursor: String
        $oid: Int!
        $showLocationDetail: Boolean!
...
class NameIncorrectOriginalSpellings extends React.Component<
  NameIncorrectOriginalSpellingsProps,
  {
    showLocationDetail: boolean;
    showCitationDetail: boolean;
    showCollectionDetail: boolean;
...
> {
  constructor(props: NameIncorrectOriginalSpellingsProps) {
    super(props);
    const {
      showLocationDetail,
      showCitationDetail,
  ...
  renderInner(name: Omit<NameIncorrectOriginalSpellings_name, "oid" | " $refType">) {
    ...
    return (
      <NameIncorrectOriginalSpellingsContainer
        nameInner={name}
        title={title}
        subtitle={subtitle}
        hideTitle={hideTitle}
  ...
  render() {
    ...
    if (
      ...
      return (
        <QueryRenderer<NameIncorrectOriginalSpellingsDetailQuery>
          environment={environment}
          query={graphql`
            query NameIncorrectOriginalSpellingsDetailQuery(
              $oid: Int!
              $showLocationDetail: Boolean!
              $showCitationDetail: Boolean!
              $showCollectionDetail: Boolean!
