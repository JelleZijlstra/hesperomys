...
import { ArticleNeotypeDesignations_article } from "./__generated__/ArticleNeotypeDesignations_article.graphql";
import { ArticleNeotypeDesignations_articleInner } from "./__generated__/ArticleNeotypeDesignations_articleInner.graphql";
import { ArticleNeotypeDesignationsDetailQuery } from "./__generated__/ArticleNeotypeDesignationsDetailQuery.graphql";
...
interface ArticleNeotypeDesignationsInnerProps {
  articleInner: ArticleNeotypeDesignations_articleInner;
  title?: string;
  hideTitle?: boolean;
  numToLoad?: number;
...
> {
  render() {
    ...
    if (
      !articleInner.neotypeDesignations ||
      articleInner.neotypeDesignations.edges.length === 0
    ) {
      return null;
    }
    return (
...
const ArticleNeotypeDesignationsContainer = createPaginationContainer(
  ArticleNeotypeDesignationsInner,
  {
  ...
  {
    getConnectionFromProps: (props) => props.articleInner.neotypeDesignations,
    ...
    query: graphql`
      query ArticleNeotypeDesignationsPaginationQuery(
        $count: Int!
        $cursor: String
        $oid: Int!
        $showLocationDetail: Boolean!
...
interface ArticleNeotypeDesignationsProps {
  article: ArticleNeotypeDesignations_article;
  title?: string;
  hideTitle?: boolean;
  numToLoad?: number;
  hideClassification?: boolean;
...
class ArticleNeotypeDesignations extends React.Component<
  ArticleNeotypeDesignationsProps,
  {
    showLocationDetail: boolean;
    showCitationDetail: boolean;
    showCollectionDetail: boolean;
...
> {
  constructor(props: ArticleNeotypeDesignationsProps) {
    super(props);
    const {
      showLocationDetail,
      showCitationDetail,
  ...
  renderInner(
    ...
    return (
      <ArticleNeotypeDesignationsContainer
  ...
  render() {
    ...
    if (
      ...
      return (
        <QueryRenderer<ArticleNeotypeDesignationsDetailQuery>
          environment={environment}
          query={graphql`
            query ArticleNeotypeDesignationsDetailQuery(
              $oid: Int!
              $showLocationDetail: Boolean!
              $showCitationDetail: Boolean!
              $showCollectionDetail: Boolean!
