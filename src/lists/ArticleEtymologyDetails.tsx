...
import { ArticleEtymologyDetails_article } from "./__generated__/ArticleEtymologyDetails_article.graphql";
import { ArticleEtymologyDetails_articleInner } from "./__generated__/ArticleEtymologyDetails_articleInner.graphql";
import { ArticleEtymologyDetailsDetailQuery } from "./__generated__/ArticleEtymologyDetailsDetailQuery.graphql";
...
interface ArticleEtymologyDetailsInnerProps {
  articleInner: ArticleEtymologyDetails_articleInner;
  title?: string;
...
class ArticleEtymologyDetailsInner extends React.Component<ArticleEtymologyDetailsInnerProps> {
  render() {
    ...
    if (
      !articleInner.etymologyDetails ||
      articleInner.etymologyDetails.edges.length === 0
    ) {
      return null;
    }
    const inner = (
      <>
        ...
        />
        <NameList
          connection={articleInner.etymologyDetails}
...
const ArticleEtymologyDetailsContainer = createPaginationContainer(
  ArticleEtymologyDetailsInner,
  {
  ...
  {
    getConnectionFromProps: (props) => props.articleInner.etymologyDetails,
    ...
    query: graphql`
      query ArticleEtymologyDetailsPaginationQuery(
        $count: Int!
        $cursor: String
        $oid: Int!
        $showLocationDetail: Boolean!
...
interface ArticleEtymologyDetailsProps {
  article: ArticleEtymologyDetails_article;
  title?: string;
  subtitle?: JSX.Element;
  hideTitle?: boolean;
  numToLoad?: number;
...
class ArticleEtymologyDetails extends React.Component<
  ArticleEtymologyDetailsProps,
  {
    showLocationDetail: boolean;
    showCitationDetail: boolean;
    showCollectionDetail: boolean;
...
> {
  constructor(props: ArticleEtymologyDetailsProps) {
    super(props);
    const {
      showLocationDetail,
      showCitationDetail,
  ...
  renderInner(article: Omit<ArticleEtymologyDetails_article, "oid" | " $refType">) {
    ...
    return (
      <ArticleEtymologyDetailsContainer
  ...
  render() {
    ...
    if (
      ...
      return (
        <QueryRenderer<ArticleEtymologyDetailsDetailQuery>
          environment={environment}
          query={graphql`
            query ArticleEtymologyDetailsDetailQuery(
              $oid: Int!
              $showLocationDetail: Boolean!
              $showCitationDetail: Boolean!
              $showCollectionDetail: Boolean!
